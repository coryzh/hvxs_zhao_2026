...
def process_single_source(source_data):
    """Process a single source row and return the result row"""
    (row_idx, row), id_x, pm_and_position_cols, bailer_jones_cols, method = (
        source_data
    )
    ...
    for opt in ["vpec", "vspace"]:
        ...
        gamma_min_med, gamma_min_lo_err, gamma_min_hi_err = get_errors(
            gamma_min
        )
...
def run_computation(
        ...
        survey_name: str = None, batch_size: int = 1000, n_workers: int = 4
) -> None:
    ...
    task_args = [
        ((i, row), id_x, pm_and_position_cols, bailer_jones_cols, method)
