...
def correct_zp(df: pd.DataFrame, verbose: bool = False) -> pd.DataFrame:
    """
    Correct the parallax for zero-point offset. The corrected parallax column
    is named as "parallax_corr", it contains the corrected parallax values when
    available, otherwise the original parallax values are used.
    ...
    -------
    df : pd.DataFrame
        The dataframe containing the corrected parallax, which is named as
        "parallax_corr".
    """
    ...
    logger.log(
        f"{sum(~has_valid_parallax_corr)} rows have NA parallax_corr, but "
        "they have valid parallaxes, so the parallax_corr is replaced with "
        "parallax values for these rows and are flagged by "
        "has_valid_parallax_corr.\n"
    )
