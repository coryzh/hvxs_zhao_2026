...
def clean(df: pd.DataFrame, verbose: bool = False) -> pd.DataFrame:
    hdul_halo_ids = fits.open(str(config.DATA_DIR.parent / "others" / "catalogues"
    ...
    logger.log(f"Joining the loaded catalogue with the halo source_id catalogue ...\n")
    df_joined = pd.merge(df, df_halo_ids, left_on="source_id", right_on="GaiaDR3", how="inner")

    logger.log(f"{df_joined.shape[0]} found in the halo source catalogue.\n")

    logger.log(f"Removing likely halo sources ... \n")
...
def main() -> None:
    ...
    df_cleaned.to_csv(in_file.parent / f"{in_file.stem}.csv".replace("stage_9", "stage_9a"))
