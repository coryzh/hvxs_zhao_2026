...
def clean(df: pd.DataFrame, prob_thresh: float = 0.1, opt: str = "lmc", verbose: bool = False) -> pd.DataFrame:
    """
    Used to remove members in SMC or LMC (specified by the opt argument), using the Jimenez-Arranz+23a, b
    ...
    prob_thresh : float
        Threshold of membership probability. Sources with probability above this threshold will be dropped.

    opt : str
        Used to select which Magellanic cloud members to be removed, either 'lmc' or 'smc'.
    ...
    logger.begin()
    df_mc = pd.read_csv(config.DATA_DIR.parent / "others" / "catalogues" / f"{opt.lower()}_members.csv")
    logger.log(f"{opt.upper()} member catalogue loaded, totalling {df_mc.shape[0]} rows.\n")
    ...
    # source_ids not in the SMC or LMC catalogue have empty P values, and they are considered non-members, so .isna()
    # is combined with the threshold condition.
    _filter = (df_merged["P"] <= prob_thresh) | (df_merged["P"].isna())
    n_member = df.shape[0] - _filter.sum()
    logger.log(f"{n_member} matches are considered {opt.upper()} members (prob>={prob_thresh:.1e}).\n")
