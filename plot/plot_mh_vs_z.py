...
def add_data(axs: Dict[str, plt.Axes]) -> None:
    ...
    df_hvx = pd.read_csv(
        config.RESULTS_CATALOGUE_DIR / "high-v_sources" / "combined_vpec_lolim_gt_150_unique_stage_9.csv")
    df_control = pd.read_csv(config.RESULTS_CATALOGUE_DIR / "control_sample" / "control_sample_stage_10.csv")
    ...
    bins_x = np.linspace(df_control.mh_gspphot.min(), df_control.mh_gspphot.max(), 25)
    bins_y = np.linspace(0, 10, 25)

    for key, val in style_dict.items():
        df_indiv = df_dict[key]
        coords = SkyCoord(df_indiv.ra.values * u.deg, df_indiv.dec.values * u.deg,
