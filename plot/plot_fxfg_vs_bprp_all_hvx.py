...
#
# color_dict = {
#     "csc": colors[0], "xmm": colors[1], "erass": colors[2], "swift":
# colors[3]
# }
#
# label_dict = {
...
def add_hvx(
        in_file_csv: Path, fig: plt.Figure, axs: np.ndarray[plt.Axes]) -> None:

    df = pd.read_csv(in_file_csv)
    df = clean_for_fxfg_vs_bprp(df, verbose=False)
    df_prime = pd.read_csv(
        in_file_csv.parent / f"{in_file_csv.stem}_prime.csv"
    )

    df["vpec_min_lolim"] = df["vpec_min_med"] - df["e_vpec_min"]
    df_prime["vpec_min_lolim"] = (
        df_prime["vpec_min_med"] - df_prime["e_vpec_min"]
    )
    ...
    for i, ax in enumerate(axs.flatten()):
        ...
        # colorbar's height top_box = ax.get_position()  # Get the position of
        # the top-right subplot bottom_box = ax.get_position()  # Get the
        # position of the bottom-right subplot

        # Calculate the position and dimensions for the colorbar
        # left = top_box.x0 + 0.6  # Slightly to the right of the rightmost
        # subplot bottom = top_box.y0  # Bottom edge aligned with the bottom
        # subplot height = 0.015  # Covering the height of both rows
        # width = (top_box.x1 - top_box.x0) * 0.5  # Fixed width for the
        # colorbar
        cax = ax.inset_axes([0.55, 0.1, 0.4, 0.05])
        _cbar = fig.colorbar(scatter, cax=cax, orientation="horizontal")
        _cbar.set_label(r"$v_\mathrm{pec, min, lo}$", labelpad=10)
...
def axes_settings(fig: plt.Figure, axs: np.ndarray[plt.Axes]) -> None:
    ...
    fig.text(
        0.05, 0.5, r"$F_X/F_G$",
        ha='center', va='center', rotation=90, fontsize=50
    )
...
# / "high-v_sources" / "combined_vpec_med_gt_200_unique_cleaned.csv")
...
#
#         ax.errorbar(x=x, y=y, xerr=0, yerr=med_fx_fg_err, marker="s",
# mfc="k", mec="k",
...
#     ax.set_yticklabels([r"$10^{-5}$", r"$10^{-4}$", "0.001", "0.01", "0.1",
# "1", "10"])
#
