...
def make_figure(df: pd.DataFrame, out_file: Path = None) -> None:
    ...
    y_target = df["lum_x"]
    y_target_err = df["lum_x_err"]

    ax.scatter(x_all, y_all, s=0.6, marker=".", fc="k", ec="k")
    ax.errorbar(x_target, y_target, yerr=0, marker="o", ms=8.0, mfc="limegreen", mec="k",
    ...
    ax.set_xlabel(r"$1\,\sigma$ lower limit on $v_\mathrm{pec, min}\,(\mathrm{km~s^{-1}})$")
...
def main() -> None:
    ...
    make_figure(df_targets, out_file=config.RESULTS_FIGURES_DIR / "high-v_sources"/ "vlt_p115" / "lx_vs_vpec_min.pdf")
