...
def make_figure(df: pd.DataFrame, out_file: Path = None) -> None:
    ...
    ax.errorbar(
        x_target, y_target, yerr=y_target_err, marker="o", ms=13.0,
        mfc="limegreen", mec="k",
    ...
    ax.set_yticklabels(
        [r"$10^{-5}$", r"$10^{-4}$", "0.001", "0.01", "0.1", "1", "10"]
    )
...
def main() -> None:
    ...
    out_file = (
        ...
        / "high-v_sources" / "vlt_p115" / "fxfg_vs_bprp.pdf"
    )

    make_figure(
        df_targets, out_file=out_file
    )
